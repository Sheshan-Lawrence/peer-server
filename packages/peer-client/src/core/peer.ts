...
export class Peer extends Emitter<PeerEvent> {
  ...
  constructor(
    ...
    this.pc.onnegotiationneeded = async () => {
      if (this._closed || this._makingOffer) return;
      try {
  ...
  send(data: any, channel?: string): void {
    ...
    if (data instanceof ArrayBuffer) {
      ch.send(new Uint8Array(data) as unknown as ArrayBufferView<ArrayBuffer>);
  ...
  restartIce(): void {
    ...
    try {
      ...
      this.pc.createOffer({ iceRestart: true }).then((offer) => {
        if (this._closed || this.pc.signalingState !== 'stable') return;
